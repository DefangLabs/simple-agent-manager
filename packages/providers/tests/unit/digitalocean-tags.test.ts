...
import {
  delimitedTagsToLabels,
  hasAmbiguousLabel,
  kvTagsToLabels,
  labelsToDelimitedTags,
  labelsToKvTags,
} from '../../src/kv-tags';
...
describe('DigitalOcean colon tags', () => {
  it('round-trips values by splitting only on the first colon', () => {
    expect(
      digitalOceanTagsToLabels(labelsToDigitalOceanTags({ env: 'prod', ref: 'abc:def' }))
    ).toEqual({ env: 'prod', ref: 'abc:def' });
  });
  it.each([{ bad: 'space value' }, { bad: 'equals=value' }, { ['x'.repeat(256)]: 'y' }])(
    'fails fast for unencodable labels',
    (labels) => expect(() => labelsToDigitalOceanTags(labels)).toThrow(/Cannot encode label/)
  );
