...
describe('HetznerProvider lifecycle', () => {
  ...
  describe('getVM', () => {
    ...
    it('should fail fast on malformed provider payloads', async () => {
      globalThis.fetch = vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify({ server: { id: 12345, name: 'missing-fields' } }), {
            status: 200,
          })
        );
  ...
  describe('listVMs', () => {
    const mockServers = {
      servers: [
        createMockServer({
          id: 1,
          name: 's1',
          public_net: { ipv4: { ip: testIpv4(1, 1, 1, 1) } },
        ...
        createMockServer({
          ...
          name: 's2',
          status: 'off',
          public_net: { ipv4: { ip: testIpv4(2, 2, 2, 2) } },
    ...
    it('should return list of VMs without labels', async () => {
      globalThis.fetch = vi
        .fn()
        .mockResolvedValue(new Response(JSON.stringify(mockServers), { status: 200 }));
    ...
    it('follows Hetzner server pagination and preserves label filters', async () => {
      ...
      expect(firstUrl.searchParams.get('label_selector')).toBe(
        'managed-by=simple-agent-manager,node=n1'
      );
      expect(secondUrl.searchParams.get('page')).toBe('2');
      expect(secondUrl.searchParams.get('label_selector')).toBe(
        'managed-by=simple-agent-manager,node=n1'
      );
    });
    ...
    it('fails closed when Hetzner server pagination exceeds the max-page guard', async () => {
      ...
      await expect(provider.listVMs()).rejects.toThrow(
        new RegExp(`exceeded ${DEFAULT_HETZNER_MAX_LIST_PAGES} pages`)
      );
  ...
  describe('validateToken', () => {
    it('should return true for valid token', async () => {
      globalThis.fetch = vi
        .fn()
        .mockResolvedValue(new Response(JSON.stringify({ datacenters: [] }), { status: 200 }));
