...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_BATCH_BYTES`            | `2097152`                    | Maximum legacy `tool_metadata` bytes read into JS by one automatic archival cleanup alarm batch                                                                                  |
...
| `PROJECT_DATA_TOOL_PAYLOAD_ARCHIVE_WRITE_TIMEOUT_MS`       | `5000`                       | Per-R2-write timeout for automatic archival cleanup; timeout leaves the original payload in ProjectData and defers retry                                                         |
