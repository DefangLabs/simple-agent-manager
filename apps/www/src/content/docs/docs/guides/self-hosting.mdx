...
2. Provisions infrastructure via Pulumi (D1, KV, R2, DNS), including prefix-scoped R2 retention for session snapshots, private VM diagnostic evidence, one-day temporary uploads, and thirty-day TTS cache objects (`infra/resources/storage.ts:r2BucketLifecycle`)
...
config override. There is deliberately no age-only rule for durable `library/` data or
...
For guided recovery, run the **D1 Time Travel Restore** workflow (`d1-restore.yml`) with the printed timestamp. Always preview first with `dry_run=true`; select `main`, `observability`, or `both`, then repeat with `dry_run=false` only after confirming the target. For example:
