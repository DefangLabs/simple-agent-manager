...
describe('ProjectData alarm ordering', () => {
  it('runs storage safety before heavier lifecycle maintenance', () => {
    ...
    const alarmBody = source.slice(alarmStart, source.indexOf('async fetch(', alarmStart));
    const storageSafety = alarmBody.indexOf('storageSafety.runProjectDataStorageSafetyAlarm');
